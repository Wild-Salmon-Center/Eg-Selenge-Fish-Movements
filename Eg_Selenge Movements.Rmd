...
pacman::p_load(tidyverse, sf, readxl, riverdist, janitor, maptiles, tidyterra, ggpattern, lubridate)
```

```{r Read in data}

# Read in all fish movements 
allfish <- read_csv("All Fish Movements_Eg_Selenge.csv")%>% 
  clean_names() %>% 
...
species_colors <- c("royalblue1", "yellow2", "chocolate", "olivedrab3", "white")
...
ggplot(allfish_plotdata, aes(x = reorder(as.factor(id),max_river_distance_km), 
  ...
  labs(#title = "Fish Tracking Maximum Displacement",
       ...
       fill = "Species:") +
  geom_text(aes(label = paste0("(",months_range$partial_months,")")), 
            vjust = -0.5,
            size = 3.5) +
  geom_text(aes(label = numb_geolocations$count), 
            vjust = -2, 
            fontface = "bold",
            size = 3.5) +
  theme_bw() +
...
ggsave(filename = "AllFish_MaxDisplacement_Plot.png", height = 6, width = 12)
